...
async def run_compliance(ctx: PipelineContext) -> AsyncIterator[str]:
    ...
    notes_str = ""
    if law_notes:
        notes_str = "\n\n**Regulatory constraints from graph:**\n" + "\n".join(f"- {n}" for n in law_notes)

    yield make_chat_message("assistant", (
        f"Compliance analysis complete for **{regime or 'General Best Practices'}**.\n\n"
        f"- {required_count} required controls\n"
        f"- {advisory_count} advisory controls\n"
        f"- {bp_count} best practices\n"
        f"{notes_str}\n\n"
        f"Mapping components to AWS services..."
    ))
