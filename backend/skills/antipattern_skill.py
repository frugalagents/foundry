...
async def run_antipattern_check(ctx: PipelineContext) -> AsyncIterator[str]:
    ...
    status_parts = []
    if blocked:
        status_parts.append(f"**{len(blocked)} blocked** (requires immediate attention)")
    if warnings:
        status_parts.append(f"**{len(warnings)} warnings** (monitor closely)")
    if prevented:
        status_parts.append(f"**{len(prevented)} mitigated** by selected components")

    if not antipatterns:
        status_msg = "No anti-patterns detected for your configuration."
    else:
        status_msg = "Detected: " + ", ".join(status_parts)

    yield make_chat_message("assistant", (
        f"Anti-pattern analysis complete.\n\n"
        f"{status_msg}\n\n"
        f"Building your phased implementation roadmap..."
    ))
