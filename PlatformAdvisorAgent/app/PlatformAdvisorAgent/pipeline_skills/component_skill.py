...
async def run_component_selection(ctx: PipelineContext) -> AsyncIterator[str]:
    ...
    yield make_panel_complete(3, "architecture_diagram", {
        ...
        "pattern": pattern_name,        # frontend ArchitectureDiagramData.pattern
    ...
    yield make_chat_message("assistant",
        f"**{len(components)} components** mapped across {len(layers)} layers "
        f"({elevated_count} tier-elevated). See Architecture Diagram →"
    )
