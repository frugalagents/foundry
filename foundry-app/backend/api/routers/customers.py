...
async def list_customers(user: CurrentUser):
    ...
    if not is_admin(user):
        items = [i for i in items if i.get("created_by") == actor_id or i.get("demo_data")]
    return [_to_customer(i) for i in items]


@router.post("", response_model=Customer, status_code=status.HTTP_201_CREATED)
async def create_customer(body: CustomerCreate, user: CurrentUser):
    item = db.create_customer(name=body.name, created_by=get_user_id(user))
...
async def delete_customer(customer_id: str, user: CurrentUser):
    ...
    if not db.delete_customer(customer_id, owner_id=get_user_id(user)):
        raise HTTPException(status_code=404, detail="Customer not found")
