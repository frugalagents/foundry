...
export function PhaseTimeline({ data, streaming }: { data: PhaseTimelineData | null; streaming: boolean }) {
  ...
  const phaseColors = ['var(--accent)', 'var(--accent-deep)', 'var(--success)', 'var(--warning)'];

  return (
    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <h2 className="text-display" style={{ fontSize: 'var(--text-lg)' }}>Implementation Roadmap</h2>
          ...
          <div key={phase.id}>
            ...
            <div
              style={{
                ...
                background: `color-mix(in srgb, ${phaseColors[pi]} 12%, transparent)`,
                border: `1px solid color-mix(in srgb, ${phaseColors[pi]} 28%, transparent)`,
                borderBottom: 'none',
                textAlign: 'center',
              }}
            >
              <div style={{ fontSize: 12, fontWeight: 700, color: phaseColors[pi] }}>{phase.id}</div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', marginTop: 2 }}>{phase.duration}</div>
            </div>
            {/* Components */}
            <div
              style={{
              ...
              {phase.components.map((comp) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 'var(--text-xs)', color: `var(--tier-${comp.tier})`, border: `1px solid var(--tier-${comp.tier})`, borderRadius: 3, padding: '1px 4px' }}>
                      T{comp.tier}
                    </span>
                    <span style={{ fontSize: 'var(--text-xs)', color: effortColors[comp.effort] }}>
                      {comp.effort} effort
                    </span>
                  </div>
                  {comp.aws_service && (
                    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 2 }}>{comp.aws_service}</div>
                  )}
                </div>
              ))}
      ...
      {data.dependencies.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <div className="eyebrow" style={{ color: 'var(--text-secondary)', marginBottom: 8 }}>
            Key Dependencies
          </div>
